...
locals {
  ...
  all_network_interfaces_map = merge([
    for vm_key, vm_value in var.az_vm_windows : {
...
resource "azurerm_windows_virtual_machine" "vm_windows" {
  ...
  admin_password = each.value.admin_password != null ? each.value.admin_password : var.windows_admin_password
...
resource "azurerm_backup_protected_vm" "recovery_vault_backup_enable" {
  ...
  resource_group_name = var.backup_resource_group_name
  recovery_vault_name = var.recovery_vault_name
  source_vm_id        = azurerm_windows_virtual_machine.vm_windows[each.key].id
